...
class BaseGraph(ABC):
    ...
    async def get_entities_to_relationships_map(self, is_directed=False, show_progress: bool = False):
        ...
        if show_progress:
            import tqdm
            iterator = tqdm.tqdm(edges, total=num_edges, desc="Building E2R map", unit="edge")
        else:
            iterator = edges

        for e_idx, (u, v) in enumerate(iterator):
            ui = node_to_idx[u]
            data.append([ui, e_idx])
            if not is_directed:
                vi = node_to_idx[v]
                if vi != ui:
                    data.append([vi, e_idx])

        return csr_from_indices(data, shape=(num_nodes, num_edges))
    ...
    async def get_relationships_to_chunks_map(self, doc_chunk, show_progress: bool = False):
        ...
        result = []
        total = len(raw_relationships_to_chunks)
        if show_progress:
            import tqdm
            iterator = tqdm.tqdm(raw_relationships_to_chunks, total=total, desc="Building R2C map", unit="rel")
        else:
            iterator = raw_relationships_to_chunks

        for chunk_ids in iterator:
            idxs = await doc_chunk.get_index_by_merge_key(chunk_ids)
            result.append([i for i in idxs if i is not None])

        return csr_from_indices_list(result, shape=(len(result), await doc_chunk.size))
    ...
    async def personalized_pagerank(self, reset_prob_chunk, damping: float = 0.1):
        ...
        if len(edges_list) > 1000:  # 如果边数量过多，使用简化权重
            ...
            igraph_.es['weight'] = edge_weights
