...
class HKDynamicAux:
    ...
    def __init__(self, workspace, shape: Tuple[int, int], force: bool = False):
        ...
        # 社区签名计算配置
        self.use_member_based_signature = True  # 是否使用基于成员的签名计算
        self.signature_aggregation_method = 'average'  # 'average' 或 'voting'
    ...
    def compute_community_signature_from_members(self, community_id: str, member_nodes: List[str]) -> int:
        """
        通过计算社区成员签名的平均值来确定社区签名
        
        Args:
            community_id: 社区ID
            member_nodes: 社区成员节点列表
