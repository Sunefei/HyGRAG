...
class FaissIndex(BaseIndex):
    ...
    async def _update_index(self, datas: list[dict[str:Any]], meta_data: list):
        ...
        #dimensions = 1024 #TODO
        sample_embedding = self.embedding_model._get_text_embedding("sample text")
        dimensions = len(sample_embedding)
        vector_store = FaissVectorStore(faiss_index=faiss.IndexHNSWFlat(dimensions, 32)) #TODO        
        #vector_store = FaissVectorStore(faiss_index=faiss.IndexHNSWFlat(self.embedding_model.dimensions, 32)) #TODO
