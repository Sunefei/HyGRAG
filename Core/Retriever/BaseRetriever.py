...
class BaseRetriever(ABC):
    ...
    async def _run_personalized_pagerank(self, query, query_entities):
        ...
        if self.config.use_entity_similarity_for_ppr:
            ...
            for entity in query_entities:
                # Skip None entities or entities without entity_name
                if entity is None or not isinstance(entity, dict) or "entity_name" not in entity:
                    continue
    ...
    async def link_query_entities(self, query_entities):
        ...
        for query_entity in query_entities:
            ...
            # Check if node_datas is not empty and first element is not None
            if node_datas and len(node_datas) > 0 and node_datas[0] is not None:
                entities.append(node_datas[0])
            # If no valid entity found, we could append None or skip
            # Here we skip to avoid adding None values
