...
def save_checkpoint(result_dir: str, current_index: int, all_results: List[Dict], dataset_len: int):
    ...
    logger.info(f"[Checkpoint] progress {current_index + 1}/{dataset_len}, completed {len(all_results)} queries")
...
def load_checkpoint(result_dir: str):
    ...
    try:
        ...
        from Core.Common.Logger import logger
        logger.info(f"[Checkpoint] last progress was question {checkpoint_data['current_index'] + 1}/{checkpoint_data['total_length']}, "
...
def wrapper_query_with_checkpoint(query_dataset, digimon, result_dir):
    ...
    if start_index is not None:
        logger.info(f"Resuming from checkpoint, starting from question {start_index + 2}")
        start_index += 1
    else:
    ...
    if processed_queries > 0:
        ...
        logger.info("=" * 60)
        logger.info("Query phase time statistics:")
